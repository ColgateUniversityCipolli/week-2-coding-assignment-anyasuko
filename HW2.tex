...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{trap.points} \hlkwb{<-} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{99}\hlopt{*}\hldef{(delta.x)} \hlopt{+} \hldef{b}
\hldef{trap.area} \hlkwb{<-} \hlkwd{sum}\hldef{(}\hlnum{.5}\hlopt{*}\hldef{delta.x}\hlopt{*}\hlkwd{integrand}\hldef{(trap.points))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
  ...
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidial"}\hldef{)\{}
    \hldef{trap.points} \hlkwb{<-} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{99}\hlopt{*}\hldef{(delta.x)} \hlopt{+} \hldef{b}
    \hldef{area} \hlkwb{<-} \hlkwd{sum}\hldef{(}\hlnum{.5}\hlopt{*}\hldef{delta.x}\hlopt{*}\hlkwd{fnct}\hldef{(trap.points))}
